...
# Map for interactive view of concentrated disadvantage
summary(city_change$white_change)
white_bins <- c(-30, -15, -7, 0, 7, 15, 30) # Sets the interval for color scheme
white_pal <- colorBin("viridis", domain = city_change$white_change, bins = bins) # Gives diverging color scheme

# Created html popup of index of concentrated disadvantage

labels_white <- sprintf(
  "<strong>%g Change Percent</strong>",
  city_change$white_change
) %>% lapply(htmltools::HTML)

# Create leaflet map to label by geometric index

white_change_map <- leaflet() %>% setView(-78.8784, 42.8864, 10.9) %>% addTiles() %>%
  addPolygons(data = nbhds_json, fillOpacity = 0, weight = 3, opacity = 1) %>%
